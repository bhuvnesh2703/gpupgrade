...
func TestReplacePrecisionInTabDelimitedData(t *testing.T) {
	cases := []struct {
		name string
		line string
		want string
		...
		{
			name: "no replacement is performed in space delimited data",
			line: ".12 adfasdfasdf 127.90.90.1 12.12 .13",
			want: ".12 adfasdfasdf 127.90.90.1 12.12 .13",
		},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := ReplacePrecisionInTabDelimitedData(c.line)
			if got != c.want {
...
func TestReplacePrecisionInBrackets(t *testing.T) {
	cases := []struct {
		name string
		line string
		want string
		...
		{
			name: "does not replace precision values in ddl clause containing values function",
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			got := ReplacePrecisionInBrackets(c.line)
			if got != c.want {
				t.Errorf("got %v, want %v", got, c.want)
			}
		})
